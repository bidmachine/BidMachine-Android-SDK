...
public abstract class IABMeasurer implements Measurer {
    ...
    public static void initialize(@NonNull final Context context,
                                  @NonNull final String partnerName,
                                  @NonNull final String partnerVersion,
                                  @NonNull final String measurerJs,
                                  @Nullable final InitListener listener) {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (isCompatible) {
                        ...
                        if (!isActive) {
                            isActive = Omid.activateWithOmidApiVersion(Omid.getVersion(),
                        ...
                        if (isActive) {
                            ...
                            if (listener != null) {
                                listener.onInitializeFail("Omid is not activated");
                            }
                        }
                    } else {
                        if (listener != null) {
                            listener.onInitializeFail("Omid is not compatible");
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    if (listener != null) {
                        listener.onInitializeFail("Internal error");
                    }
                }
            }
        });
    ...
    @MainThread
    protected abstract AdSessionConfiguration createAdSessionConfiguration() throws Exception;

    @MainThread
    protected abstract AdSessionContext createAdSessionContext(@NonNull Partner partner,
                                                               @NonNull View view) throws Exception;

    @Override
    @MainThread
    @CallSuper
    public boolean configure(@NonNull Context context, @NonNull final View view) {
        if (!isInitialized()) {
            return false;
        }
        try {
            ...
            adSessionContext = createAdSessionContext(partner, view);
    ...
    @Nullable
    protected AdSession getAdSession() {
        return adSession;
    }

    @Nullable
    protected AdEvents getAdEvents() {
        return adEvents;
    }
    ...
    @Override
    @MainThread
    public void removeIgnoredView(@NonNull View view) {
        try {
            if (adSession != null) {
